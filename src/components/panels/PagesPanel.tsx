...
export const PagesPanel = () => {
  const { activeProject, activePageId, setActivePageId, addPage, deletePage } = useApp();
          ...
          {pages.map(page => (
            <div
              ...
              className={`flex justify-between items-center rounded text-sm p-1.5 cursor-pointer ${
                page.id === activePageId ? 'bg-juki-green/20 text-juki-green' : 'text-gray-300 hover:bg-juki-dark-3'
              }`}
            >
              <div>
           ...
           {pages.length === 0 && activeProject && (
              <p className="text-xs text-center text-gray-500 py-4">This project has no pages.</p>
           )}
        </div>
      </div>
    </>
