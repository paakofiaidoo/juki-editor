...
export const SelectionWrapper = ({ children, item, isSelected, isHighlightedParent, onClick }: SelectionWrapperProps) => {
    ...
    useEffect(() => {
        ...
        return combine(
            draggable({
                element: el,
                getInitialData: () => ({ type: "canvas-item", itemId: item.id } as unknown as Record<string, unknown>),
            }),
    ...
    const highlightClasses = isSelected ? "selection-highlight-animated" : isHighlightedParent ? "parent-highlight" : "hover:outline hover:outline-1 hover:outline-juki-green/50";
            ...
            {isSelected && (
                <>
                    <div className="absolute -top-6 left-0 flex items-center gap-1 z-30">
                        <div className="bg-juki-green text-black text-xs font-bold px-2 py-0.5 rounded-t-md pointer-events-none">{item.name}</div>
                        <div className="flex items-center bg-juki-green rounded-t-md">
                            ...
                            <button
                                ...
                                className="p-1 text-black hover:bg-black/20"
                            >
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    openModal({ type: "CREATE_COMPONENT", sourceItemIds: [item.id] });
                                }}
                                title="Save as Component"
                                className="p-1 text-black hover:bg-black/20 disabled:text-black/50 disabled:cursor-not-allowed"
                                disabled={isLocked}
                            >
                                <Box size={14} />
                            </button>
